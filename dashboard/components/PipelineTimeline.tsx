...
export default function PipelineTimeline({ timeline }: Props) {
  ...
  const settledCount = Object.values(services).filter(
    (s) => s.scanEndMs !== null,
  ).length;
  ...
  const settledMax = Math.max(
    ...
    ...Object.values(services).map(
      (s) =>
        (s.mrOpenedMs ?? s.scanEndMs ?? s.scanStartMs ?? startMs) - startMs,
    ),
    0,
  );
  ...
  function pct(ms: number | null | undefined, fallback = now): number {
    if (ms == null)
      return Math.min(((fallback - startMs!) / totalMs) * 100, 100);
      ...
      {open && (
        <div className="px-4 pb-4 flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono text-[9px] text-ripple-subtle w-32 shrink-0 truncate">
              Intelligence
            </span>
            <div className="flex-1 relative h-4 bg-white/5 rounded overflow-hidden">
              ...
              {!intelligenceEndMs && (
                <div
                  className="absolute top-0 left-0 h-full bg-ripple-accent/40 animate-pulse rounded"
                  style={{ width: "100%" }}
                />
              )}
            </div>
          ...
          {ALL_SERVICES.map((name) => {
            ...
            const scanWidth = scanStart
              ? scanEnd
                ? pct(scanEnd) - pct(scanStart)
                : pct(undefined, now) - pct(scanStart)
              : 0;
            const fixLeft = scanEnd ? pct(scanEnd) : 0;
            const fixWidth = scanEnd
              ? mrOpened
                ? pct(mrOpened) - pct(scanEnd)
                : 0
              : 0;
